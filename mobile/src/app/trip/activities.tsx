...
export function Activities({ tripDetails }: Props) {
  ...
  async function handleCraeteTripActivity(){
    try{
  ...
  async function getTripActivities(){
    try{
      ...
      const activitiesToSelectionList = activities.map((dayActivity) => ({
        title: {
      ...
      setTripActivities(activitiesToSelectionList)
  ...
  <View className="flex-1">
...

      {
        section.data.length === 0 && (
          <Text className="text-zinc-500 font-regular text-sm mb-8">
              Nnehuma atividade cadastrada nessa data.
          </Text>
        )}
    </View>
    )}
    ...
    >
      ...
      <Button 
      onPress={handleCraeteTripActivity} 
      isLoading={isCreatingActivity}
      >
        <Button.Title>Salvar atividades</Button.Title>
      </Button>
    </Modal>
